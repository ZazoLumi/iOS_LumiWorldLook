...
public protocol AGPlayerDelegate: NSObjectProtocol {
    func playerReady(_ player: AVPlayerViewController)
...
extension AGVideoPlayerView {
    fileprivate func addPlayerObservers() {
        ...
        self._timeObserver = playerController.player?.addPeriodicTimeObserver(forInterval: CMTimeMake(1, 100), queue: DispatchQueue.main, using: { [weak self] timeInterval in
            ...
            strongSelf.playbackDelegate?.playerCurrentTimeDidChange((self?.playerController)!)
        })
